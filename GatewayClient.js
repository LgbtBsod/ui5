sap.ui.define([
    ...
    function withDirectPut(sPath, vPayload, mOptions) {
        ...
        // Fetch/refresh CSRF token before the PUT - required by SAP Gateway for all mutating requests
        ...
        return pToken.then(function (sCsrfToken) {
            ...
            // Build absolute URL: serviceUrl already excludes trailing slash
            ...
            // Use native XHR to send binary payloads (File/Blob/ArrayBuffer).
            // ODataModel.update() serializes the payload as JSON, which corrupts binary data.
            return new Promise(function (resolve, reject) {
                ...
                oXhr.onreadystatechange = function () {
                    ...
                    if (oXhr.status >= 200 && oXhr.status < 300) {
                        ...
                        reject(GatewayErrorNormalizer.normalizeError(oErr));
                    }
                };
                oXhr.onerror = function () {
                    reject(GatewayErrorNormalizer.normalizeError({ statusCode: 0, message: "Network error during binary PUT" }));
                };
                // vPayload can be File, Blob, ArrayBuffer, or string - all handled natively by XHR.send()
