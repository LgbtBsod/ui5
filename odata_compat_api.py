...
def _person_full_name(person: Person) -> str:
    return " ".join(
        part.strip()
        for part in [person.last_name or "", person.first_name or "", person.middle_name or ""]
...
def service_document():
    return {"d": {"EntitySets": ["ChecklistSet", "ChecklistFlatSet", "CheckSet", "BarrierSet", "LastChangeSet", "PersonVHSet", "ChecklistPermissionSet", "AttachmentSet", "MplTreeSet"]}}


@router.get(f"{SERVICE_ROOT}/$metadata")
def metadata():
    oResponse = Response(content=get_metadata(), media_type="application/xml")
...
def person_vh(
    ...
    expression = FilterParser.parse(Person, filter, field_map=PERSON_VH_MAP)
    if expression is not None:
        query = query.filter(expression)

    active_date = _parse_query_date(date_check)
    ...
    if search:
        ...
        rows = [
            ...
            if normalized_search in _person_full_name(row).lower()
            or normalized_search in str(row.perner or "").lower()
        ]
        total = len(rows)
...
def mpl_tree_set(date_value: str = Query(..., alias="AssessDate"), db: Session = Depends(get_db)):
    ...
    results = [{"NodeId": r["NodeID"], "ParentId": r["ParentNodeID"], "Level": r["HierarchyLevel"], "Text": r["Description"], "ValidFrom": date_value, "ValidTo": ""} for r in tree]
...
def mpl_tree_function(assess_date: str = Query(..., alias="date"), db: Session = Depends(get_db)):
    ...
    return {"d": {"results": [{"node_id": r["NodeID"], "parent_id": r["ParentNodeID"], "level": r["HierarchyLevel"], "text": r["Description"], "valid_from": assess_date, "valid_to": ""} for r in tree]}}
