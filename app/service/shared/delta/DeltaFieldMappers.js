sap.ui.define([
  ...
  function toAttachmentFields(oRow, iIndex, sEditMode, sChecklistDbKey) {
    var sMode = DeltaContracts.normalizeEditMode(sEditMode, DeltaContracts.EDIT_MODE.UPDATE);
    var sKey = pickValue(oRow, IDENTITY.ATTACHMENT_KEY_FIELDS);
    var bCreate = sMode === DeltaContracts.EDIT_MODE.CREATE;
    var sAttachmentDbKey = String(pickValue(oRow, IDENTITY.ROOT_KEY_FIELDS) || sChecklistDbKey || "").trim();
    var sParentDbKey = String(pickValue(oRow, IDENTITY.PARENT_KEY_FIELDS) || sChecklistDbKey || "").trim();
    var sFolderDbKey = String(pickValue(oRow, ["FolderKey", "folderKey"].concat(IDENTITY.PARENT_KEY_FIELDS)) || sParentDbKey || "").trim();
    return {
      ...
      edit_mode: sMode,
      root_key: sAttachmentDbKey,
      parent_key: sParentDbKey,
      folder_key: sFolderDbKey,
