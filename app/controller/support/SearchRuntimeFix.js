sap.ui.define([
    ...
    function clearSyncTimers(oController) {
        oController._iSearchRuntimeFixTimer = SchedulingRuntime.clearTimer(oController._iSearchRuntimeFixTimer);
        oController._iSearchRuntimeFixLateTimer = SchedulingRuntime.clearTimer(oController._iSearchRuntimeFixLateTimer);
    }
    ...
    function scheduleSync(oController, bImmediate) {
        ...
        if (bImmediate) {
            ...
            oController._iSearchRuntimeFixTimer = SchedulingRuntime.restartTimer(oController._iSearchRuntimeFixTimer, function () {
                runSync(oController);
            }, 96);
        }
        oController._iSearchRuntimeFixLateTimer = SchedulingRuntime.restartTimer(oController._iSearchRuntimeFixLateTimer, function () {
            runSync(oController);
        }, 320);
    }
    ...
    function bindTableUpdate(oController) {
        var oSmartTable = oController && oController.byId && oController.byId("searchSmartTable");
        var oInnerTable = oSmartTable && oSmartTable.getTable && oSmartTable.getTable();
        if (!oInnerTable || oInnerTable.data("searchRuntimeFixBound")) {
            return;
        }
        ...
        oInnerTable.data("searchRuntimeFixBound", true);
    }
    ...
    function bindResizeObserver(oController) {
        ...
        if (!oController._oSearchRuntimeFixObserver) {
            oController._oSearchRuntimeFixObserver = new window.ResizeObserver(function () {
        ...
        aTargets.forEach(function (oTarget) {
            oController._oSearchRuntimeFixObserver.observe(oTarget);
        });
    }

    function bindScrollHost(oController) {
        var oScrollHost = resolveScrollHost(oController);
        if (!oScrollHost || oController._oSearchRuntimeFixScrollHost === oScrollHost) {
            return;
        }
        if (oController._oSearchRuntimeFixScrollHost && oController._fnSearchRuntimeFixScroll) {
            oController._oSearchRuntimeFixScrollHost.removeEventListener("scroll", oController._fnSearchRuntimeFixScroll, { passive: true });
        }
        oController._fnSearchRuntimeFixScroll = oController._fnSearchRuntimeFixScroll || function () {
            if (oController._iSearchRuntimeFixFrame) {
                return;
            }
            oController._iSearchRuntimeFixFrame = window.requestAnimationFrame(function () {
                oController._iSearchRuntimeFixFrame = 0;
                runSync(oController);
            });
        };
        oScrollHost.addEventListener("scroll", oController._fnSearchRuntimeFixScroll, { passive: true });
        oController._oSearchRuntimeFixScrollHost = oScrollHost;
    }
