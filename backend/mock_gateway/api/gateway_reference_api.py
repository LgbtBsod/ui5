...
from api.gateway_core import _entity_metadata, _err, _parse_odata_datetime
...
def dictionary_item_set(
    ...
    if filter:
        try:
            expr = FilterParser.parse(
                DictionaryItem, filter, field_map={"Domain": "domain", "Key": "key", "Text": "text"}
            )
    ...
    rows = [
        _to_dict_row(r.domain, r.key, r.text)
        for r in query.order_by(asc(DictionaryItem.domain), asc(DictionaryItem.key)).all()
    ]

    config_rows = [
        _to_dict_row(r.get("Domain", ""), r.get("Key", ""), r.get("Text", "")) for r in _dictionary_config_rows(db)
    ]
    if filter:
        try:
...
def person_vh_set(
    ...
    if search:
        ...
        rows = [
            row for row in rows if sQuery in _person_full_name(row).lower() or sQuery in str(row.perner or "").lower()
        ]
    total = len(rows)
    top, skip = parse_paging(top, skip, DEFAULT_PAGE_SIZE)
    return odata_payload(
        [_to_person_vh(row) for row in rows[skip : skip + top]], total if inlinecount == "allpages" else None
    )
