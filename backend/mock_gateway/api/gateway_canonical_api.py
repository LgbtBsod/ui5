...
def _to_current_user(profile: dict) -> dict:
    ...
    return {
        ...
        "PermissionsCsv": ",".join(permissions),
        "CanView": "view" in permissions,
        "CanEdit": "edit" in permissions,
        "CanDelete": "delete" in permissions,
...
def _current_user_summary(profile: dict) -> str:
    permission_labels = {
        "01": "Ð¿Ñ€Ð¾ÑÐ¼Ð¾Ñ‚Ñ€",
        "02": "Ñ€ÐµÐ´Ð°ÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ðµ / ÑÐ¾Ð·Ð´Ð°Ð½Ð¸Ðµ",
        "03": "ÑƒÐ´Ð°Ð»ÐµÐ½Ð¸Ðµ",
    }
    labels = []
    ...
    if not labels:
        return "Ð£ Ð²Ð°Ñ Ð½ÐµÑ‚ Ð½Ð°Ð·Ð½Ð°Ñ‡ÐµÐ½Ð½Ñ‹Ñ… Ð¿Ð¾Ð»Ð½Ð¾Ð¼Ð¾Ñ‡Ð¸Ð¹."
    return "Ð£ Ð²Ð°Ñ ÐµÑÑ‚ÑŒ Ð¿Ð¾Ð»Ð½Ð¾Ð¼Ð¾Ñ‡Ð¸Ñ Ð½Ð° " + " / ".join(labels) + "."
...
async def lock_acquire_function_import(request: Request, root_id: str | None = Query(None, alias="RootId"), session_guid: str | None = Query(None, alias="SessionGuid"), uname: str | None = Query(None, alias="Uname"), payload: dict | None = None, db: Session = Depends(get_db)):
    body = payload or {}
    if not body:
        try:
    ...
    resolved_root_id = root_id or merged.get("RootId") or merged.get("root_id") or merged.get("RootKey")
...
async def lock_heartbeat_function_import(request: Request, root_id: str | None = Query(None, alias="RootId"), session_guid: str | None = Query(None, alias="SessionGuid"), uname: str | None = Query(None, alias="Uname"), payload: dict | None = None, db: Session = Depends(get_db)):
    body = payload or {}
    if not body:
        try:
            body = await request.json()
...
async def lock_release_function_import(request: Request, root_id: str | None = Query(None, alias="RootId"), session_guid: str | None = Query(None, alias="SessionGuid"), uname: str | None = Query(None, alias="Uname"), payload: dict | None = None, db: Session = Depends(get_db)):
    body = payload or {}
    if not body:
        try:
            body = await request.json()
...
async def copy_checklist(request: Request, root_id: str | None = Query(None, alias="RootId"), session_guid: str | None = Query(None, alias="SessionGuid"), uname: str | None = Query(None, alias="Uname"), payload: dict | None = None, db: Session = Depends(get_db)):
    body = payload or {}
    if not body:
        try:
            body = await request.json()
            ...
            body = {}
    merged = _merge_query_and_payload(request, body)
    resolved_root_id = root_id or merged.get("RootId") or merged.get("SourceRootKey") or merged.get("source_root_key") or merged.get("SourceUuid")
    resolved_session_guid = session_guid or merged.get("SessionGuid") or merged.get("session_guid")
...
def set_status(payload: dict, response: Response, if_match: str | None = Header(None, alias="If-Match"), db: Session = Depends(get_db)):
    ...
    new_status = _normalize_status_input(payload.get("NewStatus") or payload.get("new_status") or payload.get("Status") or payload.get("status") or "")
