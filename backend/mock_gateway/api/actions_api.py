...
from api._hierarchy import hierarchy_odata_results_response
from utils.odata import odata_entity

router = APIRouter(prefix="/actions", tags=["Actions"])


def _raise_http_error(status: int, exc: ValueError) -> None:
    raise HTTPException(status_code=status, detail=str(exc)) from exc


def _value_or_http(status: int, operation: Callable[[], Any]) -> Any:
    try:
        return operation()
    except ValueError as exc:
        _raise_http_error(status, exc)


def _odata_entity_or_http(status: int, operation: Callable[[], Any]) -> dict[str, Any]:
    return odata_entity(_value_or_http(status, operation))


@router.post("/{root_id}/set-status")
def set_status(root_id: str, status: str, user_id: str, db: Session = Depends(get_db)):
    return _value_or_http(404, lambda: ActionService.set_status(db, root_id, status, user_id))


@router.post("/SubmitChecklist")
def submit_checklist(root_id: str, user_id: str, db: Session = Depends(get_db)):
    return _odata_entity_or_http(404, lambda: ActionService.set_status(db, root_id, "SUBMITTED", user_id)["status"])


@router.post("/UnlockChecklist")
def unlock_checklist(root_id: str, user_id: str, db: Session = Depends(get_db)):
    _value_or_http(409, lambda: LockService.release(db, root_id, user_id))
    return odata_entity(True)
...
def save_checklist(root_id: str, user_id: str, payload: dict, force: bool = False, request_guid: str | None = None, db: Session = Depends(get_db)):
    return _odata_entity_or_http(
        409,
        lambda: ChecklistService.save_via_import(
            db,
            root_id,
            user_id,
            payload,
...
def get_mpl_hierarchy(date: str, db: Session = Depends(get_db)):
    return hierarchy_odata_results_response(db, date)
