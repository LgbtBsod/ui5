...
class LockService:
    ...
    @staticmethod
    def lock_expires_at(lock: LockEntry | None):
        """Calculate lock expiration time based on last refresh.

        Args:
            lock: LockEntry instance or None

        Returns:
            Datetime when lock expires, or None if lock is None
        """
    ...
    def _is_expired(lock: LockEntry | None, current_time=None) -> bool:
        ...
        lock_expires_at = LockService.lock_expires_at(lock)
    ...
    def _lock_payload(
        ...
        owner_session: str,
        lock_expires,
        is_killed_flag: bool = False,
        lock_refreshed: bool = False,
        owner_session_match: bool | None = None,
        action: str = "",
    ) -> dict:
        return {
            "ok": bool(success),
    ...
    @staticmethod
    def active_lock(db: Session, db_key: str) -> LockEntry | None:
        """Get active non-expired lock for a checklist.

        Args:
            db: Database session
            db_key: Checklist database key

        Returns:
            Active LockEntry or None if no active lock exists

        Side effects:
            Marks expired locks as killed and logs expiration
        """
        ...
        if existing.session_guid == session_guid:
            ...
            return LockService._with_server_state(
                LockService._lock_payload(
                    ...
                    "LOCK_OK",
                    existing.user_id,
                    existing.session_guid,
                    LockService.lock_expires_at(existing),
                    False,
                    True,
                    True,
                    "HEARTBEAT",
        ...
        if force_takeover and (not steal_from or steal_from == existing.session_guid):
            ...
            return LockService._with_server_state(
                LockService._lock_payload(
                    ...
                    uname,
                    session_guid,
                    LockService.lock_expires_at(new_lock),
                    True,
                    True,
                    True,
                    "ACQUIRED",
    ...
    def status(db: Session, db_key: str, session_guid: str) -> dict:
        active_lock = LockService.active_lock(db, db_key)
        if not active_lock:
    ...
    @staticmethod
    def release(
        db: Session, db_key: str, session_guid: str, try_save: bool = False, payload: dict | None = None
    ) -> dict:
        lock = (
            db.query(LockEntry)
            .filter(
        ...
        if try_save and isinstance(payload, dict):
            try:
                ...
                ChecklistService.save_via_import(
                    db, db_key, lock.user_id, payload, is_autosave=False, force=True, session_guid=session_guid
                )
                s_save_status = "S"
            except Exception:
                db.rollback()
    ...
    def cleanup(db: Session) -> int:
        ...
        for lock in expired:
            ...
            db.add(
                LockLog(
                    db_key=lock.db_key, user_id=lock.user_id, session_guid=lock.session_guid, action="CLEANUP_EXPIRED"
                )
            )
    ...
    def validate_session_lock(db: Session, db_key: str, session_guid: str) -> bool:
        lock = LockService.active_lock(db, db_key)
        if not lock:
            raise ValueError("LOCK_MISSING")
