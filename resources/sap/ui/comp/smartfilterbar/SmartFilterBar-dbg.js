...
sap.ui.define([
	...
	SmartFilterBar.prototype.applyVariant = function (oVariant, sContext, bInitial) {
		...
		if (!sContext) {
			this._resetFiltersInErrorValueState(true /* Skip custom controls */);
		}
	...
	SmartFilterBar.prototype._onLiveChange = function(oEvent) {
		...
		var currentValue = oControl.getValue && oControl.getValue();

		// Clear validation error only if the input is empty
		if (oControl.data("__validationError") && !currentValue) {
			oControl.data("__validationError", null);
			oControl.setValueState(ValueState.None);
			delete oControl.__sValidationText; // BCP: 1970006434
			// Do NOT clear oControl.__lastInvalidValue here
			return;
		}

		// Do not change anything if the control is in warning state
		if (oControl.getValueState && oControl.getValueState() === ValueState.Warning) {
			return;
		}

		// Preserve error state if the value matches the last invalid value
		if (oControl.__lastInvalidValue && currentValue === oControl.__lastInvalidValue) {
			oControl.setValueState && oControl.setValueState(ValueState.Error);
		}
	};
