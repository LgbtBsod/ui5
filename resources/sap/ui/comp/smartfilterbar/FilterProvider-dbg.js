...
sap.ui.define([
	...
	var FilterProvider = function(mPropertyBag) {
		...
		this._sInputsWithCompositeKeysData = {};
			...
			if (oControl instanceof MultiInput) {
				...
				while (i--) {
					const oToken = new Token(),
						  aInputsWithCompositeKeys = Object.keys(this._getInputsWithCompositeKeysData()),
						  oItem = aItems[i],
						  sText = oItem.text || oItem.key,
						  sName = oFilterFieldMetadata?.name;

					if (aInputsWithCompositeKeys?.includes(sName)) {
						const aSelectedItems =  this._getInputsWithCompositeKeysData(sName);

						aSelectedItems.forEach((oSelectedItem) => {
							if (
								oSelectedItem.key === oItem.key &&
								oItem.text?.includes(oSelectedItem.text)
							){
								oToken.data("compositeKey", oSelectedItem.compositeKey);
							}
						});
					}
	...
	FilterProvider.prototype._handleMultiInput = function(oControl, oFieldViewMetadata, oType) {
		...
		if (oFieldViewMetadata.hasValueListAnnotation || (oFieldViewMetadata.type === "Edm.String") || bDateFormat) {
			...
			oControl.attachEvent("_validateOnPaste", function(oEvent) {
				...
				// When only 1 text exists ... we have one row with multiple columns in clipboard so we get the key value and set is as plain text input
				// Later validation will be done and a token will be created based on the defined text arrangement
				if (iLengthRows <= 1 && iLengthTexts > 1) {
					sText = Array.isArray(aTextsByRows) && Array.isArray(aTextsByRows[0]) && aTextsByRows[0][0];
					if (sText) {
				...
				// The tokens will be created without respect of the defined text arrangement
				// Instead the pasted values will be added as tokens from ValuHelpDialog's "Define Conditions" dialog
				if (iLengthRows > 1) {
							...
							if (mConditionTypeFields[sField].isParameter) {
								...
								if (oRange) {
									if (oNewValue.conditionTypeInfo.data.operation === 'DATE') {
										...
										oNewValue.conditionTypeInfo.data.value1 = oRange.value1;
										oNewValue.conditionTypeInfo.data.value2 = oRange.value2;
									}
								}
							}
...
// continue;
	...
	FilterProvider.prototype._setInputsWithCompositeKeysData = function (oData) {
		this._sInputsWithCompositeKeysData = Object.assign({}, oData);

		return this;
	};

	FilterProvider.prototype._setInputsWithCompositeKeysFieldData = function(sFieldName, aArray) {
		this._sInputsWithCompositeKeysData[sFieldName] = aArray;

		return this;
	};

	FilterProvider.prototype._getInputsWithCompositeKeysData = function(sFieldName) {
		if (sFieldName) {
			return this._sInputsWithCompositeKeysData[sFieldName];
		}
	...
	FilterProvider.prototype.destroy = function() {
		...
		this._sInputsWithCompositeKeysData = null;
		this._oNotValidatedSingleFields = {};
		this._oSingleFieldSetFromFilterData = {};
