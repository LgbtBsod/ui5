...
sap.ui.define([
	...
	var SmartTable = VBox.extend("sap.ui.comp.smarttable.SmartTable", /** @lends sap.ui.comp.smarttable.SmartTable.prototype */ {
		metadata: {
			...
			events: {
				...
				/**
				 * This event is fired right before the export is triggered. Listeners can prevent the default action of this event by calling the <code>preventDefault</code> method of the event object.
				 *
	...
	*/
	...
	SmartTable.prototype._updateNoDataText = function(bUseInitial, bHasFilters) {
		...
		if (vNoData?.isA?.("sap.m.IllustratedMessage")) {
			const oInnerMessage = this.getTable()?.getNoData();

			if (this._sLastNoDataTitle !== vNoData.getTitle()) { // Prevent overwriting custom IllustratedMessage
				if (vNoData !== oInnerMessage) {
					// Only set IllustratedMessage if the inner table's noData is not already an IllustratedMessage
	...
	SmartTable.prototype._updateQuickGroup = function(oColumn) {
		...
		if (oColumnInfo.displayBehaviour !== "idOnly" && oColumnInfo.description) {
			...
			var sSortProperty = this._getColumnSortProperty(oAdditionalPropertyColumn);
			if (sSortProperty && oAdditionalPropertyColumn) {
				this._oQuickGroup.addItem(new QuickGroupItem({
					key: sSortProperty,
		...
		if (oColumnInfo.unit) {
			...
			var sSortProperty = this._getColumnSortProperty(oAdditionalPropertyColumn);
			if (oAdditionalPropertyColumn && sSortProperty) {
				this._oQuickGroup.addItem(new QuickGroupItem({
					key: sSortProperty,
					label: this._getColumnLabel(oAdditionalPropertyColumn),
					grouped: oAdditionalPropertyColumn.data("p13nData").grouped
	...
	SmartTable.prototype.exit = function() {
		...
		TableUtil.cleanupPluginsBeforeDestroy(this);
