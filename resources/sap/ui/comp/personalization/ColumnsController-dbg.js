...
sap.ui.define([
	...
	ColumnsController.prototype._onColumnMove = function(oEvent) {
		...
		oControlData.columns.columnsItems = this._reorderColumnsItems(aColumnsItems, iIndexFrom, iIndexTo);
	...
	ColumnsController.prototype._reorderColumnsItems = function(aColumnsItems, iIndexFrom, iIndexTo){
		let iStep = 0;
		...
		const aMItem = aColumnsItems.splice(iIndexFrom, 1);
		aColumnsItems.splice(iIndexTo, 0, aMItem[0]);

		for (let index = iIndexTo; index != iIndexFrom; index = index + iStep) {
	...
	ColumnsController.prototype.getUnionData = function(oJsonBase, oJson) {
		...
		Object.keys(oJsonBase.columns).forEach(function(sAttribute) {
			...
			if (oUnion.columns[sAttribute] === undefined && oJsonBase.columns[sAttribute] !== undefined) {
				if (sAttribute !== "fixedColumnCount") {
					// we skip fixedColumnCount override in this case as it comes "deleted" and exists in the base
