...
sap.ui.define([
	...
	DateRangeType.SingleParameterOperations = {
		DATE: Object.assign({},
	...
	DateRangeType.SingleOperations = Object.assign({},DateRangeType.SingleParameterOperations,
		{
			NODATE:{
				key: "NODATE",
				operator: "NODATE",
				category: "DATERANGETYPE_NODATECATEGORY",
				type: "nodate",
				order:100,
				textKey:"DATERANGETYPE_NODATE",
				value1: null,
				defaultValues: [null]
			}});
	...
	DateRangeType.prototype.getOperations = function() {
		if (this.oFieldMetadata?.isParameter && this._bSingleFilterRestriction) {
			return this.getSingleParameterOperations();
		}
	...
	DateRangeType.prototype.getSingleParameterOperations = function() {
		var aOperations = [];
		for (var n in DateRangeType.SingleParameterOperations) {
			var oOperation = DateRangeType.SingleParameterOperations[n];
			if (this._filterOperation(oOperation)) {
				aOperations.push(oOperation);
			}
		}
	...
	DateRangeType.prototype.initialize = function(oJson) {
		...
		if (!oJson.value2 && oJson.operation === "DATERANGE") {
			oJson.operation = "DATE";
		}
	...
	DateRangeType.prototype.getFilterRanges = function() {
		var oSecondDate,
			...
			oRange;
		if (this._bSingleFilterRestriction){
			if (oCondition.operation !== "NODATE") {
		...
		if (oCondition.operation === "DATE") {
			...
			if (oCondition.operation === "NODATE"){
				oCondition.operation = "EQ";
				oCondition.value1 = null;
			} else if (!(this.isValidCondition() && oCondition.value1)) {
				return [];
			}
			...
			oCondition.operation = "BT";
		} else if (oCondition.operation === "NODATE") {
			oCondition.operation = "EQ";
			oCondition.value1 = null;
		} else {
			if (oRange) {
	...
	DateRangeType.prototype._createCustomDDOption = function (oCustomOperation) {
		...
		} else if (oCustomOperation.type === "nodate"){
			oCustomDynamicDateOption = this._createNoDateDDOption(oCustomOperation);
		} else {
	...
	DateRangeType.prototype._createNoDateDDOption = function (oOperation) {
		const sOptionId = oOperation.key,
		DynamicDateOptionClass = DynamicDateOption.extend(sOptionId, {
			metadata: {
				library: "sap.ui.comp"
			},
			getText: function () {
				return Type.getTranslatedText(oOperation.textKey);
			},
			getValueHelpUITypes: function () {
				return [];
			},
			format: function () {
				return Type.getTranslatedText("DATERANGETYPE_NODATE_TEXT");
			},
			toDates: function () {
					return [];
			},
			parse: function(sValue) {
				if (sValue === Type.getTranslatedText("DATERANGETYPE_NODATE_TEXT") || sValue === Type.getTranslatedText("DATERANGETYPE_NODATE")) {
					return oOperation;
				}
		...
		return new DynamicDateOptionClass({
			key: sOptionId
		});
	};
