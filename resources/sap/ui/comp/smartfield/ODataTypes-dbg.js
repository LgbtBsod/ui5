...
sap.ui.define([
	...
	ODataTypes.prototype.getUOMDisplayFormatter = function(oProperty, mSettings) {
		...
		return function(vMeasure, sUnit, mCustomUnits) {
			...
			const iScaleProperty = parseInt(oFormat?.oConstraints?.scale),
				sDecimalSeparator = oFormat?.oOutputFormat?.oFormatOptions?.decimalSeparator;
			if (sValue && iUnitDigits && iUnitDigits > iScaleProperty && sDecimalSeparator) {
				const aValue = sValue?.split(sDecimalSeparator);
				if (aValue.length === 2 &&  /^0*$/.test(aValue[1])) {
					if (iScaleProperty === 0){
						sValue = aValue[0];
					} else {
						const iLength = sValue.length - iUnitDigits + iScaleProperty;
						sValue = sValue.slice(0, iLength);
					}
				}
			}
