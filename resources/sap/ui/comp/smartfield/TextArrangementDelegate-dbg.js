...
sap.ui.define([
			...
			if (oModel && oBaseControlBinding) {
				...
				if (bInnerControlHasTextArrangement) {
					...
					if (oType.oFormatOptions.textArrangement === "descriptionOnly") {
						...
						if (vDescriptionValue &&
							[vDescriptionValue, vKeyValue].includes(this._sTextArrangementLastReadValue)) {
								bValueIsRead = true;
						}
					} else {
	...
	/**
	 * Creates model bindings for the InParameters and listens for changes.
	 * @param {object} oSettings The settings object
	 * @private
	 */
	TextArrangementDelegate.prototype.inParametersChange = function(oSettings) {
		var sMainPropertyPath = this.oSmartField?._oFactory?._oMetaData.path,
		oInParameters = this.oSmartField._oFactory?._oMetaData.annotations?.valueListData?.inParams,
		aInParameters = [],
		oBinding,
		oModel = this.oSmartField.getModel(),
		oBindingContext = this.oSmartField.getBindingContext();

		const fnChangeHandler = function () {
			this.oSmartField?._oFactory?.fetchIDAndDescriptionCollectionIfRequired(oSettings);
		}.bind(this);

		if (oInParameters) {
			for (var key in oInParameters) {
				if (oInParameters.hasOwnProperty(key) && key !== sMainPropertyPath) {
					aInParameters.push({"name": key, "value": oInParameters[key]});
				}
			}
		}

		if (this.aInBindings.length > 0) {
			this.aInBindings.forEach(function(oInBinding) {
				oModel.removeBinding(oInBinding);
				oInBinding.detachChange(fnChangeHandler);
			});
		}

		aInParameters.forEach(function(oParam) {
			oBinding = oModel.bindProperty(oParam.name, oBindingContext);
			oBinding.attachChange(fnChangeHandler);
			this.aInBindings.push(oBinding);
		}.bind(this));
	};
