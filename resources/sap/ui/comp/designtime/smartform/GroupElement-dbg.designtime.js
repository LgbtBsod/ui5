...
sap.ui.define([
	...
	const isCombined = (oControl) => {
		return !!oControl.data("isCombined");
	};
	...
	return {
		...
		actions: {
			...
			rename: function (oControl) {
				...
				// We offer this option only in case there is no SmartField and the feature is disabled and also for combined Group Elements
				if (containsSingleSmartField(oControl) && FeaturesAPI.areAnnotationChangesEnabled() && !isCombined(oControl)) {
					return {};
				}
			...
			annotation: function(oControl) {
				...
				// We offer this option only for non-combined already - combined case should always use basic control rename
				if (isCombined(oControl)) {
					return {};
				}
