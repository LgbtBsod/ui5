...
sap.ui.define([
...
], function(NumberFormat, DateFormat, P13nConditionPanelBase, ODataType, Currency, Unit, fnWhitespaceReplacer, FilterOperator) {
	"use strict";
	...
	var FormatUtil = {
		...
		getFormattedRangeText: function(sOperation, sValue1, sValue2, bExclude) {
			return P13nConditionPanelBase.getFormatedConditionText(sOperation, sValue1, sValue2, bExclude);
		},
		...
		/**
		 * Returns an object containing key-value pairs from a composite key path.
		 * @param {string} sLongKey The path containing composite keys formatted as "EntitySetName(Key1='Value1',Key2='Value2')"
		 * @returns {Object} mapping keys to their corresponding values.
		 * @private
		 */
		 _extractCompositeKey: function(sLongKey) {
			if (!sLongKey) {
				return;
			}
			const regPattern = /\(([^]+)\)/;
			const sMatch = regPattern.exec(sLongKey);
			if (!sMatch || !sMatch[1]) {
				return;
			}
			return sMatch[1].split(",").reduce((oKey, sItem) => {
				const [sRawKey, sRawValue] = sItem.split("=");

				if (!sRawKey || !sRawValue) {
					return oKey;
				}

				const bIsNull = sRawValue === "null",
					  bIsUndefined = sRawValue === "undefined",
					  bIsNumeric = !bIsNull && !bIsUndefined && sRawValue.trim() !== "" && !sRawValue.includes("'");
				let sParsedValue;
				if (bIsNull) {
					sParsedValue = null;
				} else if (bIsUndefined) {
					sParsedValue = undefined;
				} else if (bIsNumeric) {
					sParsedValue = Number(sRawValue);
				} else {
					sParsedValue = sRawValue.startsWith("guid") ? sRawValue.replace(/'$/g,"") : sRawValue.replace(/^'|'$/g, "");
				}
				oKey[sRawKey] = sParsedValue;
				return oKey;
			}, {});
		},
