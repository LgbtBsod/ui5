...
sap.ui.define([
	...
	function _enhanceGroup(oGroup) {
		...
		/**
		 * @deprecated As of version 1.48.0, please do not use this property as it does not have any effect on the current layout of the <code>SmartForm</code> control
		 */
			...
			if (!this.getCustomToolbar() && !this.getCheckButton() && !this.getEditTogglable()) {
				...
				if (!this._oTitle) {
					...
					if (Title && !this._bTitleRequested) {
						var sLevel = themingParameters.get({name: "sap.ui.layout.FormLayout:_sap_ui_layout_FormLayout_FormTitleSize"}) || "H1"; // get default level from theme
