...
sap.ui.define([
	...
	ValueListProvider.prototype._onAnnotationLoad = function (mValueList) {
		...
		if (this._shouldHaveRecommendations() || this._shouldHaveHistory()) {
			this._oHistoryValuesProvider?.getHistoryEnabled().then((bEnabled) => {
				if (bEnabled) {
	...
	ValueListProvider.prototype._resetErrorState = function() {
		if (this.oControl.data("__validationError")) {
			...
			this.oControl.setValueState("None");
			this.oControl.__lastInvalidValue = null;
		}
	};
	...
	ValueListProvider.prototype._setErrorState = function() {
		...
		this.oControl.__lastInvalidValue = this.oControl.getValue();
	};
	...
	ValueListProvider.prototype._onFilterSuccess = function(oResponseData, fAsyncCallback, fnResolve, bSkipAfterTokenValidate, bIsTextOnly, aFilters, mParams) {
		...
		if (oResponseData) {
			...
			if (oResponseData.results?.length === 1) {
				this.oControl._iDescriptionCount = 1;
				...
				this.oControl._iDescriptionCount = oResponseData.results.length;
			...
			if (sInput === this._truncateSearchText(sInput)) {
				...
				if (this.mFilterInputData && this.aFilterField) {
					...
					if (this.oFilterProvider) {
						mFilterParams.useContainsAsDefault = this.oFilterProvider._bUseContainsAsDefault;
						mFilterParams.stringFields = this.oFilterProvider._aFilterBarStringFieldNames;
						mFilterParams.timeFields = this.oFilterProvider._aFilterBarTimeFieldNames;
						mFilterParams.dateTimeOffsetValueFields = this.oFilterProvider._aFilterBarDateTimeFieldNames;
						mFilterParams.dateTimeOffsetWithTimeZone = this.oFilterProvider._aFilterBarDateTimeOffsetWithTimezoneFieldNames;
						mFilterParams.viewMetadataData = this.oFilterProvider._aFilterBarViewMetadata;
					}
					aFilters = FilterProviderUtils.generateFilters(this.aFilterField, this.mFilterInputData, mFilterParams);
				}
			...
			this._oSingleFieldDescriptionRead = this.oODataModel.read("/" + this.sValueListEntitySetName, {
				...
				success: function (oResponseData) {
					...
					if (oResponseData?.results && oResponseData?.results?.length === 1) {
						...
						this.oControl._iDescriptionCount = oResponseData?.results?.length || 0;
	...
	ValueListProvider.prototype.destroy = function() {
		if (this.oControl) {
			...
			delete this.oControl.__sValidationText;
			delete this.oControl.__bValidatingToken;
			delete this.oControl._iDescriptionCount;
		}
