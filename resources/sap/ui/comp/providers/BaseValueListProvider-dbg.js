...
sap.ui.define([
...
], function(library, coreLibrary, EventProvider, ODataType, MetadataAnalyser, FormatUtil, DateTimeUtil, Log, Token, Filter, FilterOperator, TableUtil, isEmptyObject) { //
	"use strict";
	...
	BaseValueListProvider.prototype._calculateAndSetFilterOutputData = function(aData) {
		...
		if (this.mOutParams && aData && (this.oFilterProvider || this.oFilterModel)) {
						...
						while (i--) {
							...
							if (oData[sValueListFieldName]) {
								...
								if (bAddAsRange) {
									...
									// For DateRangeType restore the data as Date
									// This ensures that if there is semantic operation selected before populating from out paramter, it
									// won't be used. Instead date operation with the corresnpnonding date value will be used.
									if (oFieldData?.conditionTypeInfo?.data) {
										oFieldData.conditionTypeInfo.data.operation = "DATE";
										oFieldData.conditionTypeInfo.data.value1 = oNewData.value1;
										oFieldData.conditionTypeInfo.data.value2 = oNewData.value2;
										oFieldData.ranges = [];
									}
			...
			if (mFilterOutputData) {
				...
				if (this.oFilterProvider) {
					this._clearErrorState(mFilterOutputData);
	...
	/**
	 * Clears the error state for the fields in the filter output data.
	 * This is used to reset the value state of fields in the SmartFilterBar when the
	 * filter output data is cleared or updated.
	 * @param {Object} mFilterOutputData - The fields for which to clear the error state.
	 * @private
	 */
	BaseValueListProvider.prototype._clearErrorState = function(mFilterOutputData) {
		const aFields = Object.keys(mFilterOutputData);

		aFields?.forEach(function(sField) {
			const oFilterItem = this.oFilterProvider?._oSmartFilter?._getFilterItemByName(sField),
