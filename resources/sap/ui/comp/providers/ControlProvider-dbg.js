...
sap.ui.define([
	...
	/**
	 * Wraps the original control and binds its visibility to the unit property
	 * to show/hide it based on multi-unit support.
	 *
	 * @param {sap.ui.core.Control} oTemplate Template control that needs wrapping for multi-unit support
	 * @param {object} oViewField The view field
	 * @returns {sap.ui.core.Control} The wrapped control
	 */
	ControlProvider.prototype._addMultiUnitSupport = function(oTemplate, oViewField) {
		oTemplate.bindProperty("visible", {
			path: oViewField.unit,
			formatter: MultiUnitUtil.isNotMultiUnit
		});

		// Create the MultiUnit Link
		const oAnalyticalMultiUnitLink = new Link({
		...
		return new VBox({
			renderType: "Bare",
			items: [
				oAnalyticalMultiUnitLink, oTemplate
			]
		});
	};
	...
	ControlProvider.prototype._createObjectStatusTemplate = function(oViewField, oType, oCriticalityInfo) {
		...
		if (this._isAnalyticalTable && oViewField.unit) {
			oTemplate = this._addMultiUnitSupport(oTemplate, oViewField);
		}
