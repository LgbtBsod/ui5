...
sap.ui.define([
	...
	const {IllustratedMessageType, FlexAlignItems} = MLibrary;
	...
	SmartChart.prototype._initialiseInnerChart = function() {
		...
		if (this._oChartProvider) {
			...
			if (this._oChartViewMetadata) {
				...
				this._oChart.attachRenderComplete(this._toggleZoomButtonEnablement, this);
				// Attach to sap.chart.Chart's private _selectionDetails event
			...
			if (sErrorType) {
				...
				if (sErrorType === Constants.ERROR_TYPE.NODATA) {
					...
					if (iUnavailableIndex !== -1) {
						...
						if (that._getVisibleMeasures()?.length === 0) {
							oError["Measure"] = 0;
						}
						sDescription = that._errorMessage = MDCChartUtil.getErrorMessageForMissingMeasuresAndDimensions(sChartType, oError);
						that._bInvalidChartType = true;
					}
					aContent = that._getIMButtons();

					that._calculateSettingsButtonVisibility(aContent);
				}
	...
	SmartChart.prototype._calculateSettingsButtonVisibility = function(aContent) {
		const oSettingsButton = aContent[1];
		if (this._getVisibleMeasures()?.length === 0) {
			oSettingsButton.setVisible(false);
		} else {
			oSettingsButton.setVisible(true);
		}
	};
	...
	SmartChart.prototype._updateVizTooltipFormatter = function() {
		...
		if (this._vizTooltip && this._oChart.getVizProperties().tooltip.formatString !== null) {
