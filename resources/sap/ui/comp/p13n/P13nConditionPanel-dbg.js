...
sap.ui.define([
	...
	P13nConditionPanel.prototype._createPaginatorToolbar = function() {
		...
		this._oAddButton = new Button({
			text: this._oRbComp.getText("VALUEHELPDLG_CONDITIONPANEL_ADD_CONDITION"),
			visible: true,
			press: function(oEvent) {
				that._focusFirstFE();
	...
	P13nConditionPanel.prototype._addButtons = function (oConditionGrid, oTargetGrid) {
		...
		var oRemoveControl = new Button({
			...
			ariaLabelledBy: function () {
				that._setRemoveBtnIMText(oConditionGrid);
				return that._oRemoveBtnInvisibleText.getId();
			}
		});
		const fnBtnFocusIn = oRemoveControl.onfocusin;
		oRemoveControl.onfocusin = function (oEvent) {
			fnBtnFocusIn.call(oRemoveControl);
			oEvent.stopPropagation();
			that._updateRemoveBtnAriaLabelledBy(oConditionGrid);
		};
		...
		var oControl,
			...
			params = {
				...
				change: function(oEvent) {
					...
					that._updateRemoveBtnAriaLabelledBy(oConditionGrid, oEvent);
				},
				layoutData: new GridData({
	...
	*/
	...
	/**
	 * Sets the text of the ariaLabelledBy of the remove button in the condition grid
	 *
	 * @private
	 * @param {sap.ui.layout.Grid} oConditionGrid The condition grid
	 *
	 */
	P13nConditionPanel.prototype._setRemoveBtnIMText = function(oConditionGrid) {
			const oInvisibleText = this._calcRemoveBtnIMText(oConditionGrid);
			this._oRemoveBtnInvisibleText.setText(oInvisibleText);
	};

	/**
	 * Calculates the invisible text for the remove button in the condition grid
	 * @private
	 *
	 * @param {sap.ui.layout.Grid} oConditionGrid The condition grid
	 * @returns {string} The calculated ariaLabel text
	 */
	P13nConditionPanel.prototype._calcRemoveBtnIMText = function(oConditionGrid) {
		...
		const sInputValue1 = getControlValue(oConditionGrid.value1);
		const sInputValue2 = getControlValue(oConditionGrid.value2);
		const bInvalidState = oConditionGrid?.value1?.getValueState() === ValueState.Warning ||
									oConditionGrid?.value1?.getValueState() === ValueState.Error ||
									oConditionGrid?.value2?.getValueState() === ValueState.Warning ||
									oConditionGrid?.value2?.getValueState() === ValueState.Error;
		const sSelectedKey = oConditionGrid?.operation?.mProperties?.selectedKey;

		const sRemoveConditionText = "VALUEHELPDLG_CONDITIONPANEL_REMOVE_CONDITION";

		if (!sInputValue1 && !sInputValue2) {
			return this._oRbComp.getText(sRemoveConditionText + "_UNDEFINED");

		} else if (bInvalidState) {
			return this._oRbComp.getText(sRemoveConditionText + "_INVALID");
		} else {
			const sOperation = sSelectedKey.indexOf("Not") > -1 ? sSelectedKey.slice(3) : sSelectedKey;
			const sTokenText = P13nConditionPanelBase.getFormatedConditionText(
					sOperation,
					sInputValue1, sInputValue2,
					this._oOperationsHelper.isExcludeType(sSelectedKey),
					oConditionGrid.keyField.getSelectedKey(), oConditionGrid.showIfGrouped.getSelected()
			);
			return this._oRbComp.getText(sRemoveConditionText + "_NAME", [sTokenText]);
		}
	};

	/**
	 * Updates the ariaLabelledBy association of the remove button in the condition grid
	 *
	 * @private
	 * @param {sap.ui.layout.Grid} oConditionGrid The condition grid
	 */
	P13nConditionPanel.prototype._updateRemoveBtnAriaLabelledBy = function(oConditionGrid) {
		const oRemoveControl = oConditionGrid["remove"];
		this._setRemoveBtnIMText(oConditionGrid);
		const sAriaLabelTextId = this._oRemoveBtnInvisibleText.getId();

		if (!oRemoveControl.getAriaLabelledBy().includes(sAriaLabelTextId)){
			oRemoveControl.addAriaLabelledBy(sAriaLabelTextId);
		}
	};
