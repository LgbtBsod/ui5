...
sap.ui.define([
	...
	P13nConditionPanelBase.prototype.setConditions = function(aConditions) {
		if (!aConditions) {
			Log.error("sap.ui.comp.p13n.P13nConditionPanelBase : aCondition is not defined");
		}
			...
			switch (oKeyField.type) {
				...
				case "numc":
					if (!(oKeyField.formatSettings && oKeyField.formatSettings.isDigitSequence)) {
						Log.error("sap.ui.comp.p13n.P13nConditionPanelBase", "NUMC type support requires isDigitSequence==true!");
					...
					if (!oConstraints.maxLength) {
						Log.error("sap.ui.comp.p13n.P13nConditionPanelBase", "NUMC type support requires maxLength!");
					}
	...
	P13nConditionPanelBase.prototype.init = function() {
		...
		this._oRemoveBtnInvisibleText = new InvisibleText({id: this.getId() + "-removeBtnInvisibleText"});
		...
		this.addAggregation("content", this._oRemoveBtnInvisibleText);
							...
							if (typeof vValue === "string" && oConditionGrid.oType.getName() === "sap.ui.comp.odata.type.StringDate") {
								...
								if (typeof vValue === "string" && ["String", "sap.ui.model.odata.type.String", "sap.ui.model.odata.type.Decimal"].indexOf(oConditionGrid.oType.getName()) == -1) {
									try {
										...
										Log.error("sap.ui.comp.p13n.P13nConditionPanelBase", "Value '" + vValue + "' does not have the expected type format for " + oConditionGrid.oType.getName() + ".parseValue()");
									}
								} else {
									oControl.setValue(oConditionGrid.oType.formatValue(vValue, "string"));
								}
		...
		if (oCurrentKeyField && oCurrentKeyField.typeInstance) {
			...
			if (sCtrlType === "DateTimePicker" && oType.getMetadata().getName() === "sap.ui.model.odata.type.DateTime") {
				if (!(oType.oConstraints && oType.oConstraints.isDateOnly)) {
					Log.error("sap.ui.comp.p13n.P13nConditionPanelBase", "sap.ui.model.odata.type.DateTime without displayFormat = Date is not supported!");
							...
							if (sPastedValue) {
								...
								if (oKeyField.typeInstance) {
									...
									try {
										...
										Log.error("sap.ui.comp.p13n.P13nConditionPanelBase.onPaste", "not able to parse value " + sPastedValue + " with type " + oKeyField.typeInstance.getName());
										sPastedValue = "";
										oPastedValue = null;
									}
			...
			if (oControl.getDateValue && !(oControl.isA("sap.m.TimePicker")) && oType.getName() !== "sap.ui.comp.odata.type.StringDate") {
				...
				if (oType && oType.getName() === "sap.ui.comp.odata.type.StringDate") {
					...
					try {
						...
						Log.error("sap.ui.comp.p13n.P13nConditionPanelBase", "not able to parse value " + sValue + " with type " + oType.getName());
						sValue = "";
					}
				}
			}
	 ...
	 *
	 * @private
	 * @param {string} sOperation the operation type sap.ui.comp.p13n.P13nConditionOperation
