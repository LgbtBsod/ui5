...
sap.ui.define([
	...
	/**
	 * Sets the array of tokens. The <code>sap.m.Token</code>s are added to the dialog tokenizer.
	...
	ValueHelpDialog.prototype._isComplexKeyFound = function(oParsedKey, sRowKey) {
		for (const sKey of Object.keys(oParsedKey)) {
			const sValue = oParsedKey[sKey],
				  bIsNumber = typeof sValue === "number",
				  bIsNull = sValue === null,
				  bIsGuid = !bIsNumber && !bIsNull && String(sValue).startsWith("guid");
			let sExpectedFormat;

			if (bIsNull || bIsNumber) {
				sExpectedFormat = `${sKey}=${sValue}`;
			} else if (bIsGuid) {
				sExpectedFormat = `${sKey}=${sValue}'`;
			} else {
				sExpectedFormat = `${sKey}='${sValue}'`;
			...
			if (!sRowKey.includes(sExpectedFormat)) {
				return false;
			}
		}
		return true;
					...
					for (j = 0; j < aItems.length; j++) {
						...
						for (i = 0; i < this.oRows.aKeys.length; i++) {
							...
							if (
								...
								sKey &&
								typeof sKey === "object" &&
								Object.keys(sKey).length > 1
							) {
								bIsComplexKeyFound = this._isComplexKeyFound(sKey, sRowKey);
							}
		...
		if (this._oTable && this._oTable.isA("sap.ui.table.Table")) {
			...
			var fnChangeHandler = function(oControlEvent) {
				var bEventIsInternallyTriggered = oControlEvent.getParameter("_internalTrigger") && oControlEvent.getParameter("_internalTrigger") === true,
					...
					bHasSelectedIndex = oTableSelectionInstance?.getSelectedIndices()?.length > 0;
