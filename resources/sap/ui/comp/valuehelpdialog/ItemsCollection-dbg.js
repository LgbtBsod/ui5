...
sap.ui.define([
	 ...
	 * @param {object} oFormatter - The formatter used when parsing the tokens text
	 * @returns {sap.m.Token[]} array of tokens with the given key and the text value
	 * @public
	 */
	ItemsCollection.prototype.getSelectedItemsTokenArray = function(sKey, sDescriptionKey, sDisplayBehaviour, oFormatter) {
		var aTokens = [];
		for ( var sItemKey in this.items) {
			...
			if (typeof oItem === "string") {
				...
				if (sText === undefined) {
					...
					if (typeof sText !== "string") {
						if (oFormatter) {
							sText = oFormatter.format(oItem[sKey]);
						} else {
							sText = sDisplayKey;
						}
					}
			...
			if (typeof oItem !== "string") {
				...
				sItemKey = FormatUtil._extractCompositeKey(sItemKey);
				if (sItemKey) {
					oToken.data("compositeKey", ManagedObject.escapeSettingsValue(sItemKey));
				}
			}
			aTokens.push(oToken);
		}
