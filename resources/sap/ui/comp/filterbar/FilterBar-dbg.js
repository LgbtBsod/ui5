...
sap.ui.define([
	...
	var FilterBar = Control.extend("sap.ui.comp.filterbar.FilterBar", /** @lends sap.ui.comp.filterbar.FilterBar.prototype */
	{
		...
		constructor: function(sId, mSettings) {
			const oSettings = sId && typeof sId !== "string" ? sId : mSettings,
				mCustomData = oSettings?.customData;
			let oPreventVariantCreationData;

			if (mCustomData) {
				if (Array.isArray(mCustomData)) {
					oPreventVariantCreationData = mCustomData.find((oData) => oData.getKey() === "preventVariantCreation");
				} else {
					const sKey = mCustomData.getKey?.() || mCustomData["key"];
					oPreventVariantCreationData = sKey === "preventVariantCreation" ? mCustomData : null;
				}
			}

			if (oPreventVariantCreationData) {
				const vValue = oPreventVariantCreationData.getValue?.() || oPreventVariantCreationData["value"];
				const bValue = typeof vValue === "string" ? vValue === "true" : !!vValue;
				this._bPreventVariantCreation = !oSettings.persistencyKey && bValue;
			} else {
				this._bPreventVariantCreation = false;
			}
	...
	FilterBar.prototype.init = function() {
		...
		// Create variant layout only if not prevented by custom data
		if (!this._bPreventVariantCreation) {
	 ...
	 * @param {boolean} bIgnoreCustom Should custom controls be ignored
	 * @private
	 */
	FilterBar.prototype._resetFiltersInErrorValueState = function(bIgnoreCustom) {
		var aNameControls;

		aNameControls = this._retrieveCurrentSelectionSet(true, true);

		// SNOW: DINC0687846 - ignore custom fields
		if (bIgnoreCustom) {
			aNameControls = aNameControls.filter((oObject) => {
				const oFilterItem = this.determineFilterItemByName(oObject.name);
				return !oFilterItem?.data("isCustomField");
			});
		}
						...
						if ((sReason === "SelectAll" || sReason === "DeselectAll")) {
							...
							if (this._oAdaptFiltersPanel.getCurrentViewKey() === this._oAdaptFiltersPanel.LIST_KEY) {
								...
								for (i = 0; i < mItem.length; i++) {
									...
									if (this._shouldBeVisible(mItemInRangeSelection)) {
										bindingContextPath = "/items/" + i;
										this._oAdaptFiltersDialogModel.setProperty(bindingContextPath + "/visible", true);
									} else if (!mItemInRangeSelection.visible) {
										fItemHandler(mItemInRangeSelection, false);
									} else {
	...
	FilterBar.prototype.fireFilterChange = function(oEvent) {
		const bResetEnabled = !this._bResetFiltersDialogTriggered;
		if (!this.bPreventUpdateToolbarText) {
			this._updateToolbarText();
			this.fireAssignedFiltersChanged();
		}
		...
		if (this._getConsiderFilterChanges() && this._oVariantManagement && !this._oVariantManagement.getInErrorState()) {
			this._oVariantManagement.currentVariantSetModified(bResetEnabled);
		}
		...
		if (this._oAdaptFiltersDialog) {
			...
			this._oAdaptFiltersDialog.getResetButton?.()?.setEnabled(bResetEnabled); // enable it as even invalid should be possible to be reset
		}
	...
	* Checks if <code>sLabel</code> is contained in <code>aLabels</code>
	...
	FilterBar.prototype.exit = function() {
		...
		this._bPreventVariantCreation = null;
